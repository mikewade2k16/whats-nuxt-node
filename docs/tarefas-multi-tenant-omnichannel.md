...
### Arquitetura
- **Frontend:** Nuxt 4 + Vue 3 + Pinia (`apps/omni-nuxt-ui/`)
- **Backend API:** Fastify + Prisma + Socket.IO (`apps/api/`)
- **Platform Core:** Go (autenticacao centralizada, multi-tenant, modulos) (`apps/platform-core/`)
...
1. O **platform-core** (Go) gerencia tenants, usuarios, modulos e limites
...
4. Um **platform admin** (super root) pode trocar de tenant via `POST /auth/switch-tenant`
5. Cada tenant pode ter 1+ instancias WhatsApp, cada uma com sua conexao no Evolution API

### Modulos por cliente (platform-core)
...
- `Tenant` → id (cuid), slug, name, whatsappInstance, maxChannels, maxUsers
- `User` → tenantId (FK), email, role (ADMIN/SUPERVISOR/AGENT/VIEWER)
- `WhatsAppInstance` → tenantId (FK), instanceName, isActive, userScopePolicy
- `WhatsAppInstanceUserAccess` → tenantId, instanceId, userId
- `Conversation` → tenantId (FK), instanceId (FK), externalId, contactName
- `Message` → tenantId (FK), conversationId (FK), content, direction
...
| Card "Acesso ao Modulo Atendimento" | Sim (filtra role=ADMIN — acesso permanente) | Sim (proprios usuarios, filtra role=ADMIN) | Nao |
...
- `instanceName`: auto-gerado como `{clientSlug}-instancia-00{n}` — cliente nao ve nem edita
...
demo-instance → 4 tenants (demo, demo-core, Duby, Perola)
...
-- Webhook URL: http://api:4000/webhooks/evolution/demo-core
```
...
|---------|-----------------|
| `apps/api/src/routes/tenant/routes-whatsapp-session.ts` | Bootstrap, connect, logout de WhatsApp |
| `apps/api/src/routes/tenant/routes-whatsapp-instances.ts` | CRUD de instancias, atribuicao de usuarios |
| `apps/api/src/routes/tenant/tenant-webhook-config.ts` | Construcao de webhook URL |
| `apps/api/src/services/whatsapp-instances.ts` | Service layer de instancias |
| `apps/api/src/services/evolution-client.ts` | Cliente HTTP para Evolution API |
| `apps/api/src/lib/guards.ts` | Guards de acesso (requireAtendimentoModuleAccess) |
| `apps/api/src/services/core-atendimento-access.ts` | Resolucao de acesso via platform-core |
| `apps/api/src/routes/tenant/routes-clients.ts` | Listagem de clientes (PROBLEMA: expoe todos) |
| `apps/api/src/routes/tenant/routes-core.ts` | Config do tenant (PROBLEMA: expoe API key) |
| `apps/api/src/routes/auth.ts` | Login + switch-tenant |
| `apps/api/src/routes/webhooks/register-routes.ts` | Recebimento de webhooks |
| `apps/api/src/routes/webhooks/handlers/message-upsert/events.ts` | Publicacao de eventos realtime |
| `apps/api/src/main.ts` | Socket.IO setup, Redis pub/sub |
| `apps/omni-nuxt-ui/app/composables/omnichannel/useOmnichannelAdmin.ts` | Composable da pagina operacao |
| `apps/omni-nuxt-ui/app/composables/omnichannel/useOmnichannelInbox.ts` | Composable do inbox |
| `apps/omni-nuxt-ui/app/composables/omnichannel/useOmnichannelInboxRealtime.ts` | Socket.IO client |
| `apps/omni-nuxt-ui/app/components/omnichannel/OmnichannelInboxModule.vue` | UI do inbox |
| `apps/omni-nuxt-ui/app/components/omnichannel/OmnichannelAdminModule.vue` | UI da operacao |
| `apps/omni-nuxt-ui/app/stores/session-simulation.ts` | Switch de tenant (frontend) |
...
- **Teste:** Criar instancia para Duby com nome `duby-whatsapp` → deve criar no Evolution API separadamente
- **Teste:** Tentar criar instancia com nome `demo-instance` em novo tenant → deve recusar
...
- **Teste:** Apos bootstrap do Duby, verificar no Evolution: `GET /webhook/find/{instanceName}` → URL deve conter `duby` ou `novo-cliente-2`
...
- Script SQL para limpar:
  ...
  WHERE "tenantId" IN ('cmmtq0zu3000aqs4p594e787t', 'cmmtq18k1000lqs4puksq7ieo');
  ```
- Alternativa: criar script em `apps/api/src/scripts/cleanup-shared-instances.ts`
...
- Backend: `POST /auth/switch-tenant` verifica moduleCodes do cliente alvo
**O que falta:**
- Backend: `requireAtendimentoModuleAccess` em `guards.ts` deve tambem verificar se o TENANT do JWT tem o modulo ativo
- Isso requer consultar o platform-core ou cache local com os modulos do tenant
- **Solucao proposta:**
...
- **Teste:** Super root logado em tenant sem atendimento → API retorna 403 nas rotas `/tenant/*`
...
**Status:** FEITO (2026-03-16)
**Prioridade:** ALTA
**Problema:** A pagina `/admin/omnichannel/operacao` usa `useOmnichannelAdmin()` que le `user.tenantId` do auth store. Quando o admin faz switch-tenant, o composable nao recarrega.
**Solucao:**
...
- **Teste:** Switch de Root para Duby na operacao → deve mostrar instancias e config do Duby
...
- **Teste:** Admin do Duby faz bootstrap → cria `duby-whatsapp` no Evolution com webhook `http://api:4000/webhooks/evolution/novo-cliente-2`
...
- **Teste:** Duby gera QR → escaneia com celular → Duby recebe msgs em tempo real sem afetar Root
...
- Novo endpoint `PUT /tenant/users/:userId/atendimento-access` com body `{ grant: boolean }`
  - Arquivo: `apps/api/src/routes/tenant/routes-atendimento-users.ts`
...
- **Teste:** Admin do Duby abre operacao → vê lista de usuarios → clica "Conceder acesso" → usuario aparece em verde
...
**Prioridade:** ALTA
**O que foi feito:**
- Novo guard `requirePlatformAdmin` em `apps/api/src/lib/guards.ts`
  - Chama `requireAdmin` (verifica role JWT) + `resolveCoreAtendimentoAccessByEmail` (verifica isPlatformAdmin)
- `GET /clients` agora usa `requirePlatformAdmin` → 403 para client admins
- `GET /clients/:clientId/users` e `POST /clients/:clientId/users` agora verificam: se `clientId !== tenantId do JWT` → requer `requirePlatformAdmin`
...
- **Teste:** Admin do Duby chama GET `/clients` → 403; nao ve card "Clientes" na operacao

### 4.2 Nao expor Evolution API key para tenant admins
**Status:** PARCIALMENTE FEITO (2026-03-16) — frontend oculta, backend ainda retorna
**Prioridade:** MEDIA
...
- **Teste:** Admin do Duby chama GET `/tenant` → evolutionApiKey = null ou mascarado no JSON
...
### 4.4 Rate limit no switch-tenant
**Status:** A FAZER
**Prioridade:** BAIXA
**Solucao:**
...
> **Objetivo:** A inbox nunca ficar desatualizada — mesmo quando o socket esta "connected" mas silencioso.
...
1. Conversas mostram mensagens de ontem — nao chegam novas
...
**Por que acontece:**
- No evento `connect` do socket, `refreshConversationsFromRealtime({ force: true, reloadActive: true })` e chamado — CORRETO
...
- Resultado: inbox congela silenciosamente — nenhum refresh automatico ocorre enquanto o socket permanece conectado

**O "sent" falso:**
- O pipeline outbound cria uma mensagem otimista → chama a API → API aceita e retorna PENDING/SENT
- Se a Evolution API falhar internamente (depois de aceitar a requisicao), o status fica PENDING para sempre
- `reconcilePendingMessageStatus` existe e e chamado apenas quando `created.status === "PENDING"` — MAS se a API retorna SENT diretamente, nunca e chamado

### 5.6.1 Heartbeat de conversas com socket conectado
**Status:** FEITO (2026-03-30)
**Arquivo:** `apps/omni-nuxt-ui/app/composables/omnichannel/useOmnichannelInboxRealtime.ts`
**O que foi feito:**
- `CONNECTED_HEARTBEAT_INTERVAL_MS = 5 * 60_000` — heartbeat a cada 5 minutos
...
**Status:** FEITO (2026-03-30) — coberto pelo heartbeat (5.6.1) que usa `reloadActive: true` + pela sync guard (5.6.5)

### 5.6.4 Indicador visual de "inbox desatualizada"
**Status:** NAO IMPLEMENTADO (baixa prioridade — sync guard cobre o problema sem UI extra)

### 5.6.5 Sync Guard — verificacao de frescor a cada 20s + verificacao de envio
**Status:** FEITO (2026-03-30)
**Arquivos novos:**
- `apps/omni-nuxt-ui/app/composables/omnichannel/useInboxSyncGuard.ts` — freshness check + send verification
- `apps/omni-nuxt-ui/app/composables/omnichannel/useInboxMessageWindow.ts` — cap de 50 mensagens na janela
**O que foi feito:**
- `useInboxSyncGuard`: a cada 20s, chama `syncConversationHistory` + busca ultima msg do servidor — se nao esta local, recarrega; marca mensagens PENDING como FAILED apos 60s
- `useInboxMessageWindow`: limita o array de mensagens a 50 (trim para as mais recentes quando usuario esta no fundo)
- Integrados em `useOmnichannelInbox.ts` — `syncGuard.startSync()` em `onMounted` e `switchTenant`, `stopSync()` em `onBeforeUnmount`
...
**Status:** FEITO
**Causa identificada:** O volume `worker_node_modules` guardava o Prisma Client gerado na primeira vez que o container subiu. Quando o schema foi alterado (novo enum `WhatsAppInstanceUserScopePolicy`), o worker crashava em loop silencioso com `SyntaxError` — **sem processar nenhuma mensagem da fila**. O sistema exibia "conectado" mas nada era enviado ou recebido.
...
**Status:** FEITO
**Arquivo:** `apps/api/src/workers/outbound-worker.ts`
**Logica:**
...
1. **Uptime Kuma** (recomendado para inicio rapido):
   ...
   - Alertar quando `STALE_PENDING_DETECTED` aparecer nos logs do container `worker`
...
3. **Script simples de monitoramento** (fallback rapido em producao):
   ...
   docker compose logs worker --since=5m | grep STALE_PENDING_DETECTED && \
...
**Fix:** Mudado para `includeInactive: false` em `routes-whatsapp-status.ts`. Se nao ha instancia ATIVA no tenant, a rota retorna `{ configured: false }` → UI mostra "Nenhuma sessao ativa".
...
**Status:** FEITO (2026-03-16) — backend auto-gerar instanceName pendente (5.5)
**Prioridade:** ALTA
...
- Client admin NAO ve: campo "Instance name" (oculto — nome e gerado automaticamente pelo sistema)
- Platform admin ve todos os campos incluindo instance name
- Bug a corrigir: nao duplicar o alerta de status (aparece no topo da pagina E dentro do card — remover o do topo)

**Card "Cliente atual":**
- Client admin VE: Nome da empresa, Uso atual (texto "0/1 canais, 0/3 usuarios"), Retencao (dias), Limite upload
- Client admin NAO ve nem edita: Max canais, Max usuarios (sao limites do plano, definidos pelo contrato — NAO pelo cliente)
...
- Client admin NAO ve: texto de Webhook (ex: `Webhook: http://api:4000/webhooks/evolution/novo-cliente-2`) — e informacao de debug interno
...
- Client admin NAO ve nem edita: Nome tecnico da instancia (instanceName — gerado pelo sistema: `{slug}-instancia-001`)
...
- Client admin NAO ve nem edita: Flags (Instancia default, Instancia ativa) — controle de infraestrutura, nao do cliente
- Platform admin ve e edita TUDO

**Card "Acesso ao Modulo Atendimento":**
- Usuarios com role=ADMIN filtrados da lista (tem acesso automatico — nao faz sentido toggle para eles)
...
**Arquivos a alterar:**
- `apps/api/src/routes/tenant/routes-whatsapp-instances.ts`: tornar `instanceName` opcional no schema de criacao; gerar quando ausente
- `apps/api/src/routes/tenant/routes-whatsapp-session.ts`: mesmo para bootstrap (campo ja oculto no frontend para client admins)
- **Teste:** Client admin abre operacao → clica "Criar instancia" sem informar nome → instancia criada com nome `{slug}-instancia-001`
...
FASE 1 (CRITICO - sem isso nada funciona):
  1.1 Nomes unicos de instancia → 1.2 Webhook unico → 1.3 Limpeza de dados → 1.4 Guard de modulo

FASE 2 (ALTO - cada cliente gerencia seu WhatsApp):
  2.1 Operacao reage ao switch → 2.2 Bootstrap por cliente → 2.3 QR por cliente

FASE 4.1 (SEGURANCA - correcao rapida):
  4.1 /clients so platform admin

FASE 3 (MEDIO - gerenciamento de acesso):
  3.1 Admin gerencia acesso → 3.2 Atribuicao de usuarios → 3.3 Limites visiveis

FASE 4 restante (MEDIO - robustez):
  4.2 Mascarar API key → 4.3 Audit logging → 4.4 Rate limit

FASE 5 (BAIXO - polish):
  5.1 Indicador global → 5.2 Estados vazios → 5.3 Confirmacao
```
