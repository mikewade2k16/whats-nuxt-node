...
export function useUsersManager() {
  ...
  const { token, user, setSession, hydrate: hydrateAuth } = useAuth()
  const { token: coreToken, user: coreUser, setSession: setCoreSession, hydrate: hydrateCoreAuth } = useCoreAuth()
  ...
  function isCurrentSessionUser(record: UserItem) {
    ...
    const currentEmail = normalizeEmail(user.value?.email ?? coreUser.value?.email ?? '')
  ...
  function syncSessionFromUserRecord(record: UserItem) {
    if (!isCurrentSessionUser(record)) {
      return
    }

    if (user.value) {
      user.value.name = String(record.name || '').trim() || user.value.name
      user.value.email = String(record.email || '').trim() || user.value.email
      user.value.nick = String(record.nick || '').trim()
      user.value.profileImage = String(record.profileImage || '').trim()
      if (token.value) {
    ...
    if (coreUser.value) {
      coreUser.value.name = String(record.name || '').trim() || coreUser.value.name
      coreUser.value.email = String(record.email || '').trim() || coreUser.value.email
      coreUser.value.nick = String(record.nick || '').trim()
      coreUser.value.profileImage = String(record.profileImage || '').trim()
      if (coreToken.value) {
