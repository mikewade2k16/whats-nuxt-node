...
- Keep `apps/api` and `apps/platform-core` separated by domain.
- Share same PostgreSQL server, but isolate core in schema `platform_core`.
- Keep websocket state in process memory.
- Keep deployment simple: 1 instance of `platform-core`.

## Phase 1 (first growth)

- Add `platform-core` service to compose/infra as dedicated container.
...
- Scale `platform-core` horizontally (2+ replicas).
