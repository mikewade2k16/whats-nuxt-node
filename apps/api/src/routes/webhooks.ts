  ...
  while (queue.length > 0 && depth < 400) {
    ...
    if (isSameParticipant(candidateJid, participantJid)) {
      for (const key of candidateKeys) {
        ...
        if (!normalized || normalized.endsWith("@g.us") || normalized.endsWith("@s.whatsapp.net")) {
          continue;
        }
...
function enrichMentionMetadata(
  ...
  for (const mentionedJid of mentioned) {
    ...
    const fallbackPhone = extractPhone(normalizedJid);
    const displayName = options.isGroup
      ? extractParticipantNameFromGroupInfo(options.groupInfo, normalizedJid) ?? fallbackPhone
      : fallbackPhone;
    ...
    displayByJid[normalizedJid] = displayName;
    if (fallbackPhone) {
      displayByPhone[fallbackPhone] = displayName;
    }
  }
