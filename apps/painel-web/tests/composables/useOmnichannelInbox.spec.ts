...
function buildInstanceAccessResponse() {
  return {
    instances: [
      {
        id: "instance-1",
...
describe("useOmnichannelInbox", () => {
  beforeEach(() => {
    ...
    apiFetchMock.mockImplementation(async (path: string, options?: { method?: string; body?: Record<string, unknown> }) => {
      ...
      if (path === "/tenant/whatsapp/instances/access") {
        return buildInstanceAccessResponse();
      }
      ...
      if (path === "/conversations/sync-open") {
        return {
          ...
          updatedCount: 0,
          skippedCount: 0,
          totalConversationsAfterSync: conversations.length
        };
      }
  ...
  it("envia mencoes em grupo com metadataJson.mentions", async () => {
    apiFetchMock.mockImplementation(async (path: string, options?: { method?: string; body?: Record<string, unknown> }) => {
      ...
      if (path === "/tenant/whatsapp/status") {
        return { configured: true, instanceName: "demo-instance" };
      }
    ...
    expect((mentions?.displayByJid as Record<string, unknown>)?.["5511912345678@s.whatsapp.net"]).toBe("Mike Wade");
    expect((mentions?.displayByPhone as Record<string, unknown>)?.["5511912345678"]).toBe("Mike Wade");
