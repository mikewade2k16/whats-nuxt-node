...
export function useFilaAtendimentoOperationsRealtime(store = useFilaAtendimentoOperationsStore()) {
  ...
  async function recordRealtimeMetric(status: 'ok' | 'error', durationMs: number, storeId: string, message = '') {
    ...
    try {
      await bffFetch('/api/admin/modules/fila-atendimento/metrics-events', {
        ...
        body: {
          ...
          eventType: 'realtime',
          eventKey: 'fila.operacao.realtime.ticket',
          storeId,
          status,
          severity: status === 'ok' ? 'info' : 'warning',
          durationMs,
          summary: status === 'ok' ? 'Ticket realtime emitido.' : 'Falha ao emitir ticket realtime.',
          metadata: {
            message
          }
        }
      ...
      // Metricas nunca devem bloquear a conexao realtime.
    }
  }
  ...
  async function connect() {
    ...
    try {
      ...
      void recordRealtimeMetric('ok', durationSince(ticketStartedAt), normalizedStoreId)
      ...
      void recordRealtimeMetric('error', 0, normalizedStoreId, normalizeErrorMessage(error, 'Falha ao conectar realtime.'))
