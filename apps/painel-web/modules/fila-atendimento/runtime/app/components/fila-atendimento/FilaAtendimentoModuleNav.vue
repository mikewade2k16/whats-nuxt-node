...
const preservedQuery = computed(() => {
  if (String(route.query.scope || '').trim() !== 'all') {
    return undefined
  }
...
<template>
  <nav class="fila-module-nav" aria-label="Areas do modulo fila-atendimento">
    <NuxtLink
      ...
      :key="workspace.id"
      :to="preservedQuery ? { path: workspace.path, query: preservedQuery } : workspace.path"
      class="fila-module-nav__item"
      :class="{ 'fila-module-nav__item--active': workspace.id === activeWorkspace }"
    >
      <AppMaterialIcon class="fila-module-nav__icon" :name="workspace.icon" />
