  ...
  <article class="service-card" :data-testid="`operation-service-${service.id}`">
    ...
    <div class="service-card__body">
      <span class="queue-card__avatar queue-card__avatar--large" :style="{ '--avatar-accent': service.color }">
        <img v-if="service.avatarUrl" class="queue-card__avatar-img" :src="service.avatarUrl" :alt="service.name">
        <template v-else>{{ service.initials }}</template>
      </span>
    ...
    <button
      ...
      @click="handleFinish"
    >
      {{ busy ? 'Processando...' : 'Encerrar atendimento' }}
    </button>
  </article>
</template>
