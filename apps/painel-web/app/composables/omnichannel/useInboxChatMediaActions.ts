...
export function useInboxChatMediaActions(options: {
  ...
  async function fetchMessageMediaBlob(messageEntry: Message, disposition: "inline" | "attachment") {
    ...
    const shellContextHeaders = normalizeHeaders(
      unref(sessionSimulation.requestHeaders as HeadersInit | Record<string, string> | undefined)
    );
    ...
    const selectedTenantSlug = options.getSelectedTenantSlug();
    if (selectedTenantSlug) {
      headers.set("x-selected-tenant-slug", selectedTenantSlug);
    }
