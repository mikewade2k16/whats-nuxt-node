...
# Função para enviar alerta (Slack/Discord)
send_alert() {
    local message=$1
...
restart_container() {
    local container=$1
    log_message "WARN" "Container ${container} está unhealthy. Reiniciando..."
    
    docker-compose -f "$COMPOSE_FILE" -p "$COMPOSE_PROJECT" restart "$container"
    
    if [ $? -eq 0 ]; then
        log_message "INFO" "✅ Container ${container} reiniciado com sucesso"
    else
        log_message "ERROR" "❌ Falha ao reiniciar ${container}"
        send_alert "Falha ao reiniciar container: ${container}"
    fi
}

# Função para verificar saúde de um container
check_container_health() {
    ...
    if [ "$state" != "true" ]; then
        log_message "ERROR" "Container ${container} está DOWN"
        return 1
    fi
    ...
    if [ "$health" = "unhealthy" ]; then
        log_message "ERROR" "Container ${container} está UNHEALTHY"
        return 1
    fi
...
# Função para monitorar CPU/Memory
...
monitor_containers() {
    ...
    local containers=("postgres" "redis" "api" "platform-core" "worker" "web")
...
generate_health_report() {
    echo -e "\n${BLUE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}RELATÓRIO DE SAÚDE DOS CONTAINERS${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}\n"
