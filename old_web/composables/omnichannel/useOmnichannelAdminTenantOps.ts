...
export function useOmnichannelAdminTenantOps(options: {
  ...
  validateEvolutionEndpoints: (options?: { silent?: boolean }) => Promise<void>;
}) {
  ...
  async function loadInitialData() {
    ...
    try {
      ...
      await options.validateEvolutionEndpoints({ silent: true });
    } catch (error) {
  ...
  async function saveTenant() {
    ...
    try {
      ...
      const fallbackMaxUsers = options.tenant.value?.maxUsers ?? 2;
